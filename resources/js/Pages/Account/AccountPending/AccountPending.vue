...
import PendingBalanceIn from "@/Pages/Account/AccountPending/PendingBalanceIn.vue";
...
const tabs = ref([
    {
        title: 'BalanceIn',
        component: h(PendingBalanceIn, {
            pendingBalanceInCount: props.pendingBalanceInCount,
        }),
        value: '0',
        count: props.pendingBalanceInCount
    },
]);
            ...
            <TabList>
                <Tab v-for="tab in tabs" :key="tab.title" :value="tab.value">
                    <div class="flex items-center gap-2">
                        <span>{{ $t(`public.${formatType(tab.title).toLowerCase().replace(/\s+/g, '_')}`) }}</span>
