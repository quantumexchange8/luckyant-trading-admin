...
import Action from "@/Pages/Pamm/PendingPamm/PendingPammAction.vue";
...
const getResults = async (page = 1, paginate = 10, filterSearch = search.value, filterLeader = leader.value, filterDate = date.value, columnName = sorting.value) => {
    ...
    try {
        let url = `/pamm/getPendingPammData?page=${page}`;
...
const columns = [
    ...
    },
    {
        ...
        header: 'action',
        enableSorting: false,
        cell: ({ row }) => h(Action, {pammSubscription: row.original}),
    },
];

const exportSubscribers = () => {
    let url = `/pamm/getPendingPammData?exportStatus=yes`;
...
<template>
    <AuthenticatedLayout title="Pending PAMM">
        <template #header>
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p class="text-base font-normal dark:text-gray-400">
                        Manage all pending PAMM subscribers.
                    </p>
                </div>
            </div>
        </template>
