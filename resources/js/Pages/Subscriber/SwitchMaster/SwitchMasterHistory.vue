...
const sorting = ref();
const switchHistories = ref({data: []});
const action = ref('');
...
const getResults = async (page = 1, paginate = 10,  search = '', leader = null, date = '', status = '', columnName = sorting.value) => {
    try {
        let url = `/subscriber/getSwitchMasterData?page=${page}`;
        ...
        switchHistories.value = response.data.switchMasters;
...
const columns = [
    ...
    },
    {
        ...
        header: 'action',
        enableSorting: false,
        cell: ({ row }) => h(PendingAction, {switchMaster: row.original}),
    },
];
...
<template>
    <div class="p-5 my-8 bg-white overflow-hidden md:overflow-visible rounded-xl shadow-md dark:bg-gray-900">
        ...
        <div
            v-if="switchHistories.data.length === 0"
