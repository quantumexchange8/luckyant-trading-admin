...
import RenewalAction from "@/Pages/Subscription/PendingRenewal/RenewalAction.vue";
...
const totalApprove = ref(null);
const totalReject = ref(null);
...
const getResults = async (page = 1, paginate = 10, filterSearch = search.value, filterLeader = leader.value, filterDate = date.value, filterSubscriberStatus = subscriberStatus.value, columnName = sorting.value) => {
    ...
    try {
        let url = `/subscriptions/getPendingRenewalData?page=${page}`;
...
const columns = [
    ...
    {
        accessorKey: 'subscription.meta_login',
        header: 'live_account',
        enableSorting: false,
    },
    {
        accessorKey: 'subscription.master.trading_user.name',
        header: 'master',
        enableSorting: false,
    },
    {
        accessorKey: 'subscription.master.meta_login',
        header: 'account_no',
        enableSorting: false,
    },
    {
    ...
    },
    {
        accessorKey: 'action',
        header: 'action',
        cell: ({ row }) => h(RenewalAction, {renewal: row.original}),
    },
];

const exportSubscribers = () => {
    let url = `/subscriptions/getPendingRenewalData?exportStatus=yes`;
...
<template>
    <AuthenticatedLayout title="Pending Renewal">
        <template #header>
