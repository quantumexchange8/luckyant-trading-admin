...
const getLeaders = async () => {
    ...
    try {
        ...
        const selectedLeaderUserIds = props.announcement.leaders.map(leader => leader.id);
...
const thumbnailFile = ref(props.thumbnail[0]);
...
const submitForm = () => {
    ...
    form.post(route('announcement.updateAnnouncement'))
}
</script>

<template>
    <AuthenticatedLayout :title="$t('public.edit_announcement')">
        <template #header>
