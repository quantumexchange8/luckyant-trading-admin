...
const getResults = async (filterJoinDate = null, filterTerminationDate = null) => {
    ...
    try {
        let url = `/${props.routeName==='pamm'?'pamm':'copy_trading'}/getTerminationReportData?export=${exportTable.value}`;
        ...
        terminations.value = response.data.terminations;
    } catch (error) {
...
const exportReport = () => {
    let url = `/${props.routeName==='pamm'?'pamm':'copy_trading'}/getTerminationReportData?export=yes`;
        ...
        <template #content>
            ...
            >
                <DataTable
                    ...
                    :value="terminations"
                    :paginator="terminations?.length > 0"
                    removableSort
                    dataKey="id"
                    :rows="10"
                ...
                >
                    ...
                    <template #empty>
                        <div class="flex flex-col">
                            <span>No terminations</span>
                        </div>
                    </template>
                    <template #loading>
                        <div class="flex flex-col gap-2 items-center justify-center">
                            <Loading />
                            <span v-if="exportTable === 'no'" class="text-sm text-gray-700 dark:text-gray-300">Loading terminations</span>
                            <span v-else class="text-sm text-gray-700 dark:text-gray-300">Exporting Report</span>
                        </div>
                    </template>
                    <template v-if="terminations.length">
                        <Column
                        ...
                        >
                            ...
                            <template #body="slotProps">
                                <span class="uppercase">{{ dayjs(slotProps.data.termination_date).format('DD/MM/YYYY HH:mm:ss') }}</span>
                            </template>
                        </Column>
                        <Column
                            field="user.name"
                        ...
                        <Column
                            field="subscription_batch_amount"
                            sortable
                        ...
                        >
                            ...
                            <template #body="slotProps">
                                $ {{ formatAmount(slotProps.data.subscription_batch_amount ?? 0) }}
                            </template>
                        </Column>
                        <Column
                        ...
                        >
                            ...
                            <template #body="slotProps">
                                $ {{ formatAmount(slotProps.data.penalty_amount ?? 0) }}
                            </template>
                        </Column>
                        <Column
                            field="return_amount"
                        ...
                        >
                            <template #header>
                                <span class="block">{{ $t('public.returned') }}</span>
                            </template>
                            <template #body="slotProps">
                                $ {{ formatAmount(slotProps.data.return_amount ?? 0) }}
                            </template>
                        </Column>
                        <Column
                            field="action"
                        >
                            ...
                            <template #body="slotProps">
                                <TerminationTableAction
                                    :termination="slotProps.data"
                                />
                            </template>
                        </Column>
                    </template>
