...
const submitForm = () => {
    ...
    form.patch(route('copy_trading.subscriptionApproval'), {
        onSuccess: () => {
            closeDialog();
            form.reset();
        }
        ...
        <div class="flex flex-col items-center gap-4 divide-y dark:divide-gray-700 self-stretch">
            <div class="flex flex-col-reverse md:flex-row md:items-center gap-3 self-stretch w-full">
                <div class="flex flex-col items-start w-full">
                    <span class="text-gray-950 dark:text-white text-sm font-medium">{{ subscriber.user.name }}</span>
                    <span class="text-gray-500 text-xs">{{ subscriber.user.email }}</span>
                </div>
                <div class="min-w-[180px] text-gray-950 dark:text-white font-semibold text-xl md:text-right">
                    $ {{ formatAmount(subscriber.initial_meta_balance) }}
                </div>
            </div>
            ...
            <div class="flex flex-col gap-3 items-start w-full pt-4">
                <div class="flex flex-col md:flex-row md:items-center gap-1 self-stretch">
                    ...
                    <div class="text-gray-950 dark:text-white text-sm font-medium">
                        {{ dayjs(subscriber.created_at).format('DD/MM/YYYY HH:mm:ss') }}
                    </div>
                </div>
                <div class="flex flex-col md:flex-row md:items-center gap-1 self-stretch">
                    <div class="w-[140px] text-gray-500 text-xs font-medium">
                ...
                <div class="flex flex-col md:flex-row md:items-start gap-1 self-stretch">
                    ...
                    <div class="flex flex-col text-gray-950 dark:text-white text-sm font-medium">
                        <div>
                            ...
                            <Tag
                                :severity="getSeverity(subscriber.master.trading_user.from_account_type.slug)"
                                :value="$t(`public.${subscriber.master.trading_user.from_account_type.slug}`)"
                            />
                        </div>
                        <span class="text-gray-400">{{ subscriber.master_meta_login }}</span>
                    </div>
                </div>
                <div class="flex flex-col md:flex-row md:items-start gap-1 self-stretch">
                    <div class="w-[140px] text-gray-500 text-xs font-medium">
                    ...
                    <div class="flex flex-col text-gray-950 dark:text-white text-sm font-medium">
                        <span>{{ subscriber.first_leader_name }}</span>
                        <span class="text-gray-400">{{ subscriber.first_leader_email }}</span>
                    </div>
                </div>
            </div>
